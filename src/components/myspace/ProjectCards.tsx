...
export default function ProjectCards({ role }: ProjectCardsProps) {
  const { getProject } = useProject(false, false);

  const texts = {
    title: role === "developer" ? "Mis proyectos" : "Proyectos del equipo",

    description:
      role === "developer"
        ? "Proyectos en los que participas"
        : "Proyectos que administra tu equipo",
    ...
    open: role === "developer" ? "Abrir proyecto →" : "Ver proyecto →",
  };
    ...
    <div>
      ...
      {getProject.length === 0 ? (
        <div className="text-center py-16">
          <p className="mt-4 text-gray-400">No tienes ningún proyecto.</p>
        </div>
      ) : (
          ...
          {getProject.map((project) => (
            ...
            >
              ...
              <div className="grid grid-cols-3 mt-6 text-center">
                <div>
                  <p className="font-bold">{project.metrics.totalTasks}</p>
